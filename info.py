...
API_ID = int(environ['24027768'])
API_HASH = environ['ecc13f79b3afc016ab995f932b41b26f']
BOT_TOKEN = environ['6958998166:AAGs9XvRETWyhKaeSfWjqIMYnsjpiMJtF-k']
...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '6971377716').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002290245180').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '6971377716').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
auth_channel = environ.get('-1002290245180')
auth_grp = environ.get('-1002105275752')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None

# MongoDB information *
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://robincinema0:<password>@robincinema.3taxe7h.mongodb.net/?retryWrites=true&w=majority&appName=robincinema")
DATABASE_NAME = environ.get('DATABASE_NAME', "robincinema0")
COLLECTION_NAME = environ.get('COLLECTION_NAME', 'Telegram_files')

# LOG CHANNELS *
LOG_CHANNEL = int(environ.get('-1002253552471', 0))
LAZY_GROUP_LOGS = int(environ.get('flixabin_chat', 0))
REQ_CHANNEL = int(environ.get('robincinema'))
PRIME_MEMBERS_LOGS = int(environ.get('flixabin'))
...
LZURL_PRIME_USERS = [int(lazyurlers) if id_pattern.search(lazyurlers) else lazyurlers for lazyurlers in environ.get('LZURL_PRIME_USERS', '6971377716').split()]
...
# Others
TUTORIAL = environ.get('TUTORIAL', 'https://t.me/robincinema/8') # Tutorial video link for opening shortlink website 
IS_TUTORIAL = bool(environ.get('IS_TUTORIAL', True))
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'azillahmahim')
P_TTI_SHOW_OFF = is_enabled((environ.get('P_TTI_SHOW_OFF', "False")), False)
IMDB = is_enabled((environ.get('IMDB', "True")), True)
SINGLE_BUTTON = is_enabled((environ.get('SINGLE_BUTTON', "False")), False)
CUSTOM_FILE_CAPTION = environ.get("CUSTOM_FILE_CAPTION", "⚡<b>File uploaded by [Join](https://t.me/robincinema)</b>⚡\n\n📂<b>File Name:</b> ⪧ {file_caption} \n <b>Size: </b>{file_size}\n\n❤")
BATCH_FILE_CAPTION = environ.get("BATCH_FILE_CAPTION", CUSTOM_FILE_CAPTION)
IMDB_TEMPLATE = environ.get("IMDB_TEMPLATE", "<b>Your Query: {query}</b> \n‌‌‌‌🎁Support: @azillahmahim 🎁\n\n🏷 Title: <a href={url}>{title}</a>\n🎭 Genres: {genres}\n📆 Year: <a href={url}/releaseinfo>{year}</a>\n🌟 Rating: <a href={url}/ratings>{rating}</a> / 10 \n\n♥️ we are nothing without you ♥️ \n\n💛 Please Share Us 💛\n\n⚠️Click on the button 👇 below to get your query privately")
LONG_IMDB_DESCRIPTION = is_enabled(environ.get("LONG_IMDB_DESCRIPTION", "False"), False)
SPELL_CHECK_REPLY = is_enabled(environ.get("SPELL_CHECK_REPLY", "True"), False)
MAX_LIST_ELM = environ.get("MAX_LIST_ELM", None)
INDEX_REQ_CHANNEL = int(environ.get('INDEX_REQ_CHANNEL', LOG_CHANNEL))
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', '-1002042031815')).split()]
...
ADMIN_USRNM = environ.get('ADMIN_USRNM','azillahmahim') # WITHOUT @
MAIN_CHANNEL_USRNM = environ.get('MAIN_CHANNEL_USRNM','robincinema') # WITHOUT @
DEV_CHANNEL_USRNM = environ.get('DEV_CHANNEL_USRNM','flixabin') # WITHOUT @
LAZY_YT_HANDLE = environ.get('LAZY_YT_HANDLE','flixabin')  # WITHOUT @ [  add only handle - don't add full url  ] 
MOVIE_GROUP_USERNAME = environ.get('MOVIE_GROUP_USERNAME', "flixabinchat") #[ without @ ]
...
URL_SHORTNER_WEBSITE_API = environ.get('URL_SHORTNER_WEBSITE_API', '7fcd55c9a2e0a17ad0eaf06ad0209327ced0d7d2')
lazy_groups = environ.get('LAZY_GROUPS','-1002105275752')
LAZY_GROUPS = [int(lazy_groups) for lazy_groups in lazy_groups.split()] if lazy_groups else None # ADD GROUP ID IN THIS VARIABLE 
my_users = [int(my_users) if id_pattern.search(my_users) else my_users for my_users in environ.get('MY_USERS', '6971377716').split()]
